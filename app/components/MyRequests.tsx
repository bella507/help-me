...
import { AlertOctagon, MapPin, Clock, Package, Phone, Share2, Trash2 } from 'lucide-react';
...
export function MyRequests() {
  ...
  const loadRequests = (phone: string) => {
    ...
    if (stored) {
      ...
      const userRequests = allRequests.filter((req: HelpRequest) => req.phone === phone);
  ...
  const handleShare = (request: HelpRequest) => {
    const text = `ขอความช่วยเหลือ: ${categoryLabels[request.category] || request.category}\nสถานที่: ${request.location}\nติดต่อ: ${request.phone}`;
  ...
  const getStatusBadge = (status: string) => {
    if (status === 'completed') {
      return <span className="px-2 sm:px-2.5 py-0.5 sm:py-1 rounded text-[10px] sm:text-xs bg-green-100 text-green-700">สำเร็จ</span>;
    } else if (status === 'processing') {
      return <span className="px-2 sm:px-2.5 py-0.5 sm:py-1 rounded text-[10px] sm:text-xs bg-blue-100 text-blue-700">กำลังดำเนินการ</span>;
    }
    return <span className="px-2 sm:px-2.5 py-0.5 sm:py-1 rounded text-[10px] sm:text-xs bg-orange-100 text-orange-700">รอดำเนินการ</span>;
  };

  const getUrgencyBadge = (urgency: string) => {
    if (urgency === 'high') {
      return <span className="px-2 sm:px-2.5 py-0.5 sm:py-1 rounded text-[10px] sm:text-xs bg-red-100 text-red-700">เร่งด่วน</span>;
    } else if (urgency === 'medium') {
      return <span className="px-2 sm:px-2.5 py-0.5 sm:py-1 rounded text-[10px] sm:text-xs bg-orange-100 text-orange-700">ปานกลาง</span>;
    }
    return <span className="px-2 sm:px-2.5 py-0.5 sm:py-1 rounded text-[10px] sm:text-xs bg-green-100 text-green-700">ไม่เร่งด่วน</span>;
  };
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4 sm:p-6">
          <div className="flex items-center gap-2 sm:gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-gray-500">ตรวจสอบคำขอความช่วยเหลือของคุณ</p>
            </div>
          </div>

          <div className="bg-gray-50 rounded-lg p-4 sm:p-6 border border-gray-200 text-center">
            <p className="text-sm sm:text-base text-gray-700 mb-4">กรุณากรอกเบอร์โทรศัพท์เพื่อดูคำขอของคุณ</p>
            <input
              type="tel"
              value={userPhone}
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="bg-white border border-gray-200 rounded-lg p-4 sm:p-6">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <h2 className="text-gray-900">คำขอของฉัน</h2>
              <p className="text-xs sm:text-sm text-gray-500">{requests.length} คำขอทั้งหมด</p>
            </div>
          </div>
          <button
            onClick={() => {
      ...
      {requests.length === 0 ? (
        <div className="bg-white border border-gray-200 rounded-lg p-8 sm:p-12 text-center">
          ...
          <p className="text-sm sm:text-base text-gray-500">ยังไม่มีคำขอความช่วยเหลือ</p>
          <p className="text-xs sm:text-sm text-gray-400 mt-1">คำขอที่คุณส่งจะแสดงที่นี่</p>
        </div>
      ) : (
